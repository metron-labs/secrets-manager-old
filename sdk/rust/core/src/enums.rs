...
impl FromStr for LogLevel {
    ...
    fn from_str(level: &str) -> Result<Self, Self::Err> {
        match level.to_uppercase().as_str() {
            ...
            _ => Err(KSMRError::InvalidLogLevel(level.to_string())),
        }
    }
}
...
impl From<LogLevel> for LevelFilter {
    fn from(val: LogLevel) -> Self {
        match val {
            ...
            LogLevel::NOTSET => LevelFilter::Off,
        }
    }
}

pub struct SecretsManagerLogger {
    log_level: LogLevel,
}

impl Default for SecretsManagerLogger {
    fn default() -> Self {
        Self {
...
impl Clone for SecretsManagerLogger {
    fn clone(&self) -> Self {
        SecretsManagerLogger {
            log_level: self.log_level.clone(),
        }
    }
}

// Static init for env_logger to ensure it runs once
static INIT: Once = Once::new();

impl SecretsManagerLogger {
    pub fn new_with_level_info() -> Self {
        Self::new(LogLevel::INFO)
    }

    pub fn new_with_level_debug() -> Self {
        Self::new(LogLevel::DEBUG)
    }

    pub fn new_with_level_warn() -> Self {
        Self::new(LogLevel::WARNING)
    }

    pub fn new_with_level_error() -> Self {
        Self::new(LogLevel::ERROR)
    }

    pub fn new_with_level_critical() -> Self {
        Self::new(LogLevel::CRITICAL)
    }
    ...
    pub fn log_critical(&self, message: &str) {
        log::error!("CRITICAL: {}", message); // Distinguish critical logs
    }
